...
// Cadastrar aluno
    ...
    public void atualizarAluno(AlunoModel aluno) {
        String sql = "UPDATE Aluno SET nome=?, livrosEmDivida=?, livrosEmprestados=?, livrosDevolvidosA=?, matricula=?, Turma_idTurma=?, dataDevolucao=? WHERE idAluno=?";
        try (Connection conn = ConexaoComBancoDados.conectar();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public List<AlunoModel> pesquisarAlunoPorNome(String nomeBusca) {
        List<AlunoModel> alunos = new ArrayList<>();
        String sql = "SELECT a.*, t.codigoTurma, t.turno FROM Aluno a INNER JOIN Turma t ON a.Turma_idTurma = t.idTurma WHERE a.nome LIKE ? OR a.dataDevolucao LIKE ? OR matricula LIKE ?";
